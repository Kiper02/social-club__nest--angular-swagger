...
export class UserService {
  ...
  async create(createUserDto: CreateUserDto) {
      const defaultAvatarPath = join(__dirname, '..', 'dist', 'static', 'default', 'images', 'avatar.jpg');
  ...
  async avatarEdit(changeAvatarDto: ChangeAvatarDto, file: Multer.File) {
    ...
    if(!user) {
        throw new HttpException('Пользователь с таким идентификатором не найден', HttpStatus.BAD_REQUEST)
    }
    const fileName = await this.fileService.saveFile(file, EFileType.IMAGE)
    await this.userRepository.update({avatar: fileName}, {where: {id: user.id}}) 
    const updatedUser  = await this.userRepository.findByPk(changeAvatarDto.userId)
    return updatedUser;
  }
}
